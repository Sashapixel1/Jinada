--========================================================
-- Yama Mastery Farm (Isle Champion Camp)
...
local WeaponName    = "Yama"                       -- фарм мастери именно на Yama

---------------------
-- ФЛАГИ / СОСТОЯНИЕ
---------------------
local AutoYamaMastery = false
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    AddLog("HRP найден, продолжаю фарм Yama мастери (если включен).")
end)
...
local function FarmYamaOnce()
    ...
    local ok, err = pcall(function()
        ...
        UpdateStatus("Фарм мастери Yama: " .. TargetName)
        AddLog("Нашёл " .. TargetName .. ", начинаю фарм мастери.")
        ...
        if engaged then
            if hum and hum.Health <= 0 or not target.Parent then
                AddLog("✅ " .. TargetName .. " убит, мастери Yama должна была вырасти.")
            else
    ...
    if not ok then
        AddLog("Ошибка в FarmYamaOnce: " .. tostring(err))
    end
...
---------------------
local function RunYamaMasteryLoop()
    local char = LocalPlayer.Character
    local hrp  = char and char:FindFirstChild("HumanoidRootPart")
    ...
    -- проверяем, есть ли вообще Yama в аккаунте
    if not (HasToolInCharOrBackpack(WeaponName) or HasInAccountInventory(WeaponName)) then
        UpdateStatus("Yama не найдена ни в рюкзаке, ни в инвентаре аккаунта.")
        AddLog("❌ Yama отсутствует. Сначала получи меч, потом включай фарм мастери.")
        return
    end

    -- подгружаем Yama в Backpack (если лежит в инвентаре)
...
local function CreateGui()
    ...
    Title.Text = "Yama Mastery Farm (Isle Champion Camp)"
    ...
    ToggleButton.Text = "Yama Mastery Farm: OFF"
    ...
    ToggleButton.MouseButton1Click:Connect(function()
        AutoYamaMastery = not AutoYamaMastery
        if AutoYamaMastery then
            ToggleButton.Text = "Yama Mastery Farm: ON"
            ToggleButton.BackgroundColor3 = Color3.fromRGB(0,120,0)
            NoclipEnabled = true
            StopTween     = false
            UpdateStatus("Фарм мастери Yama на Isle Champion кемпе.")
            AddLog("Yama Mastery Farm включён.")
        else
            ToggleButton.Text = "Yama Mastery Farm: OFF"
            ToggleButton.BackgroundColor3 = Color3.fromRGB(60,60,60)
            NoclipEnabled = false
            ...
            AddLog("Yama Mastery Farm выключен.")
        end
    end)

    AddLog("GUI Yama Mastery Farm загружен.")
end
