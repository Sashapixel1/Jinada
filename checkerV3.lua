...
-- Надёжный скан ReplicatedStorage на предмет "фруктов" (ограниченный)
local function scanReplicatedStorage()
    ...
    -- Быстрая проверка известных контейнеров
        ...
        if node then
            for _, child in ipairs(node:GetChildren()) do
                ...
                elseif child:IsA("StringValue") or child:IsA("ObjectValue") or child:IsA("ValueBase") then
                    add(tostring(child.Value or child.Name))
                else
                    add(child.Name)
                end
            end
    ...
    -- Ограниченный глубокий обход по GetDescendants с эвристикой
    ...
    for _, inst in ipairs(RS:GetDescendants()) do
        ...
        -- Обрабатываем типы, которые часто используются
        if inst:IsA("Tool") or inst:IsA("Model") then
            add(inst.Name)
        elseif inst:IsA("StringValue") or inst:IsA("ObjectValue") or inst:IsA("ValueBase") then
            local v = inst.Value
            if v and v ~= "" then
                add(tostring(v))
            else
            ...
            if lname:find("fruit") or lname:find("devil") or lname:find("devilfruit") then
                add(inst.Name)
            else
                -- pattern like "Light-Light", "Spike-Spike"
                if inst.Name:find("%-") and #inst.Name < 40 then
                    add(inst.Name)
                end
            end
        end
...
-- Собирает данные игрока локально
local function collectPlayerData()
    ...
    -- Beli: leaderstats или Data.Beli
    ...
    -- GUI inventory (PlayerGui.Backpack.Inventory)
    ...
    -- Stats: Data.Stats -> children with Exp/Level
    ...
    if statsRoot then
        ...
        for _, s in ipairs(statsRoot:GetChildren()) do
            ...
            -- добавляем, если хоть что-то есть
...
-- Форматирует данные в текстовый отчёт
...
local function createUI()
    ...
    -- Report area (ScrollingFrame + TextLabel inside)
    ...
    -- Функция обновления и отображения
    local function refresh()
        ...
        -- подгонка размера content и CanvasSize
